...
do $$ begin
  create type public.user_role as enum ('customer', 'rider', 'business', 'investor', 'admin');
...
begin
  ...
  if tg_op = 'INSERT' then
    if new.role in ('admin'::public.user_role, 'investor'::public.user_role) then
      raise exception 'Privileged role can only be assigned by FastFleet admin.'
        using errcode = '42501';
    end if;
    return new;
  end if;

  if new.role in ('admin'::public.user_role, 'investor'::public.user_role) and new.role is distinct from old.role then
    raise exception 'Privileged role can only be assigned by FastFleet admin.'
      using errcode = '42501';
  end if;

  if old.role in ('admin'::public.user_role, 'investor'::public.user_role) and new.role is distinct from old.role then
    raise exception 'Privileged role can only be changed by FastFleet admin.'
      using errcode = '42501';
  end if;
...
begin
  ...
  if tg_op = 'INSERT' then
    if new.account_type in ('admin'::public.user_role, 'investor'::public.user_role) then
      raise exception 'Privileged account type can only be assigned by FastFleet admin.'
        using errcode = '42501';
    end if;
  ...
  if new.account_type in ('admin'::public.user_role, 'investor'::public.user_role) and new.account_type is distinct from old.account_type then
    raise exception 'Privileged account type can only be assigned by FastFleet admin.'
      using errcode = '42501';
  end if;

  if old.account_type in ('admin'::public.user_role, 'investor'::public.user_role) and new.account_type is distinct from old.account_type then
    raise exception 'Privileged account type can only be changed by FastFleet admin.'
      using errcode = '42501';
  end if;
...
-- Investor ownership is deliberately separate from the operating rider assignment
-- above. `assigned_rider_profile_id` remains the dispatch operator; these tables
-- record bicycle ownership and its history.
create table if not exists public.investor_profiles (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint investor_profile_status_dates check ((status = 'suspended' and suspended_at is not null) or status <> 'suspended')
);
...
create unique index if not exists investor_asset_assignments_one_active_owner_idx on public.investor_asset_assignments(fleet_asset_id) where ended_at is null;
create index if not exists investor_asset_assignments_investor_active_idx on public.investor_asset_assignments(investor_profile_id, assigned_at desc) where ended_at is null;
create index if not exists investor_asset_assignments_asset_history_idx on public.investor_asset_assignments(fleet_asset_id, assigned_at desc);
create unique index if not exists investor_payout_accounts_one_active_idx on public.investor_payout_accounts(investor_profile_id) where is_active;
...
do $$
begin
  if not exists (select 1 from pg_constraint where conrelid = 'public.deliveries'::regclass and conname = 'deliveries_fleet_asset_id_fkey') then
    alter table public.deliveries add constraint deliveries_fleet_asset_id_fkey foreign key (fleet_asset_id) references public.fleet_assets(id) on delete set null;
  end if;
end;
$$;
create index if not exists deliveries_fleet_asset_completed_idx on public.deliveries(fleet_asset_id, delivered_at desc) where fleet_asset_id is not null;
...
create trigger investor_payout_accounts_set_updated_at before update on public.investor_payout_accounts for each row execute function public.set_updated_at();
...
create table if not exists public.support_tickets (
  ...
  contact_phone text,
  phone text,
  topic text not null constraint support_tickets_topic_length_check check (char_length(trim(topic)) between 1 and 80),
  tracking_code text,
...
create table if not exists public.support_messages (
  ...
  body text not null constraint support_messages_body_length_check check (char_length(trim(body)) between 2 and 2000),
  created_at timestamptz not null default now()
);

-- Existing installations receive the same F-009 input bounds without requiring
-- historical rows to validate during this schema synchronization step.
do $$
begin
...
-- F-009: the service route creates a ticket and any initial messages in one
-- idempotent database transaction. Browser roles cannot execute this function.
...
begin
  ...
  insert into public.support_tickets (
    idempotency_key, user_id, contact_name, contact_email, contact_phone,
    topic, subject, message, priority, status, assigned_admin_id, admin_notes
  )
  values (
    next_idempotency_key, next_user_id, next_contact_name, next_contact_email, next_contact_phone,
    trim(next_topic), next_subject, next_ticket_message, next_priority, 'open', null, null
  )
...
create unique index if not exists support_tickets_idempotency_key_unique on public.support_tickets(idempotency_key);
...
create policy "Users can update own profile"
  ...
  with check (
    (auth.uid() = id and role not in ('admin'::public.user_role, 'investor'::public.user_role))
...
create policy "Profiles are inserted by owner"
  ...
  with check (
    (
      user_id = auth.uid()
      and id = auth.uid()
      and account_type not in ('admin'::public.user_role, 'investor'::public.user_role)
...
-- F-009 support authorization. Anonymous callers have no table privileges or
-- policies; the public support flow is available only through the server route.
revoke all on public.support_tickets from anon;
revoke all on public.support_messages from anon;

-- RLS filters rows, not columns. Authenticated owners receive only the explicit
-- safe-column grants below; internal notes, assignment, idempotency keys, and
-- administrator sender identifiers remain unavailable to browser clients.
...
-- Cross-user administrative support access is API-only. No admin browser policy
-- can outlive requireAdminSession() checks for current Auth/profile state.
...
-- Investor access is read-only in the browser. Writes and ownership changes pass
-- through the authenticated server admin routes and the service-role functions.
drop policy if exists "Investors read own profile" on public.investor_profiles;
create policy "Investors read own profile" on public.investor_profiles for select
  using (user_id = auth.uid() or public.current_user_role() = 'admin');
drop policy if exists "Investors read own bicycle assignments" on public.investor_asset_assignments;
...
drop policy if exists "Admins read investor audit events" on public.investor_audit_events;
create policy "Admins read investor audit events" on public.investor_audit_events for select using (public.current_user_role() = 'admin');

create or replace function public.prevent_investor_assignment_history_rewrite()
returns trigger language plpgsql set search_path = public as $$
begin
  if tg_op = 'DELETE' then raise exception 'Investor asset assignment history cannot be deleted'; end if;
  if new.investor_profile_id is distinct from old.investor_profile_id or new.fleet_asset_id is distinct from old.fleet_asset_id or new.assigned_at is distinct from old.assigned_at or new.assigned_by is distinct from old.assigned_by then raise exception 'Investor asset assignment history cannot be rewritten'; end if;
  if old.ended_at is not null then raise exception 'Closed investor asset assignments cannot be changed'; end if;
  if new.ended_at is null then raise exception 'An active investor asset assignment can only be closed'; end if;
  return new;
end;
$$;
drop trigger if exists investor_assignment_history_immutable on public.investor_asset_assignments;
create trigger investor_assignment_history_immutable before update or delete on public.investor_asset_assignments for each row execute function public.prevent_investor_assignment_history_rewrite();

create or replace function public.assign_investor_asset(target_investor_profile_id uuid, target_fleet_asset_id uuid, actor_user_id uuid, reason text default null)
returns uuid language plpgsql security definer set search_path = public as $$
declare assignment_id uuid; target_status text; target_asset_type text;
begin
  if coalesce(auth.role(), '') <> 'service_role' then raise exception 'Investor ownership changes require the trusted admin service' using errcode = '42501'; end if;
  ...
  if exists (select 1 from public.investor_asset_assignments where fleet_asset_id = target_fleet_asset_id and ended_at is null) then raise exception 'This bicycle already has an active investor owner'; end if;
  insert into public.investor_asset_assignments(investor_profile_id, fleet_asset_id, assigned_by, change_reason) values(target_investor_profile_id, target_fleet_asset_id, actor_user_id, nullif(trim(reason), '')) returning id into assignment_id;
  insert into public.investor_audit_events(investor_profile_id, fleet_asset_id, actor_user_id, event_type, metadata) values(target_investor_profile_id, target_fleet_asset_id, actor_user_id, 'asset_assigned', jsonb_build_object('assignment_id', assignment_id, 'reason', nullif(trim(reason), '')));
  return assignment_id;
end;
$$;

create or replace function public.transfer_investor_asset(target_fleet_asset_id uuid, next_investor_profile_id uuid, actor_user_id uuid, reason text)
returns uuid language plpgsql security definer set search_path = public as $$
declare previous_assignment public.investor_asset_assignments%rowtype; next_status text; asset_status text; current_delivery uuid; next_assignment_id uuid;
begin
  if coalesce(auth.role(), '') <> 'service_role' then raise exception 'Investor ownership changes require the trusted admin service' using errcode = '42501'; end if;
  ...
  update public.investor_asset_assignments set ended_at = now(), ended_by = actor_user_id, change_reason = trim(reason) where id = previous_assignment.id;
  insert into public.investor_asset_assignments(investor_profile_id, fleet_asset_id, assigned_by, change_reason) values(next_investor_profile_id, target_fleet_asset_id, actor_user_id, trim(reason)) returning id into next_assignment_id;
  insert into public.investor_audit_events(investor_profile_id, fleet_asset_id, actor_user_id, event_type, metadata) values(next_investor_profile_id, target_fleet_asset_id, actor_user_id, 'asset_transferred', jsonb_build_object('previous_assignment_id', previous_assignment.id, 'assignment_id', next_assignment_id, 'reason', trim(reason)));
...
-- Investor Programme Phase 2: the isolated owner ledger, payouts, and the
-- administrator-controlled 5% maintenance reserve. This mirrors migration
-- 202609050003_investor_programme_phase2.sql for fresh environments.
