...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await db.from("support_messages").insert({ ticket_id: id, sender_type: "customer", sender_user_id: user.id, body: message, visibility: "public", message_type: "message" }).select("id, sender_type, body, created_at").single();
  ...
  await recordSupportEvent(db, { ticketId: id, actorUserId: user.id, actorType: "customer", eventType: reopening ? "CASE_REOPENED" : "CUSTOMER_MESSAGE" });
