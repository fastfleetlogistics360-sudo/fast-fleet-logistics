...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let { data: supportCase, error } = await db
    ...
    .select("id, case_number, topic, subject, message, priority, status, delivery_id, tracking_code, created_at, updated_at, last_activity_at, resolved_at, closed_at, deliveries(delivery_code, status), support_messages(id, sender_type, body, created_at)")
