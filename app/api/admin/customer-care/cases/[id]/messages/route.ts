...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: supportCase } = await db.from("support_tickets").select("id, user_id, case_number").eq("id", id).maybeSingle<any>();
  if (!supportCase) return NextResponse.json({ error: "Support case not found." }, { status: 404 });
  const { data, error } = await db.from("support_messages").insert({ ticket_id: id, sender_type: "admin", sender_user_id: admin.userId, body: reply }).select("id, sender_type, body, created_at").single();
  if (error) return NextResponse.json({ error: "Could not send the reply." }, { status: 503 });
  const now = new Date().toISOString();
  await db.from("support_tickets").update({ status: "in_progress", assigned_admin_id: admin.userId, last_activity_at: now }).eq("id", id);
