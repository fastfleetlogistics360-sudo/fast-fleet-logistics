...
| F-009 support auth | `supabase-schema.sql`, `security-remediation/migrations/202607200001_f009_support_authorization.sql`, support components/API, tests | Server-only atomic/idempotent creation, owner RLS with safe-column grants, API-only admin access, authenticated profile identity, anonymous Turnstile; anonymous follow-up intentionally unsupported | Existing tickets/messages preserved; nullable idempotency column adds no historical rewrite | Yes. Dedicated forward migration with no destructive data change | Keep browser writes denied; revoke RPC temporarily and roll application forward if needed | Behavioral API/Turnstile/idempotency tests plus guarded staging owner/anonymous RLS tests | After rate limit | Medium |
...
| F-012 rider eligibility | `app/api/rider/jobs/route.ts`, `app/api/business/orders/route.ts`, marketplace checkout, `supabase-schema.sql`, `security-remediation/migrations/202607260001_f012_rider_eligibility.sql` | Use one configurable proximity/bicycle rule in rider lists, notifications, API checks, and authoritative DB acceptance; add food/interstate marketplace policy | Valid nearby cross-state jobs work; stale/far riders, bicycle over-limit jobs, and distant fresh food are rejected; non-perishable interstate checkout requires confirmation | Yes. Apply the forward migration once after backup | Pause cross-border matching by setting Admin pickup radius to 1 km; do not remove the DB acceptance guard | Local regression/type checks plus staged route/location/bicycle scenarios | After maps, migration before application deploy | Medium |
...
F-009 repository implementation is documented in `security-remediation/03-support-authorization-remediation.md`. Production SQL has not been executed and deployment remains a separately approved operation.
