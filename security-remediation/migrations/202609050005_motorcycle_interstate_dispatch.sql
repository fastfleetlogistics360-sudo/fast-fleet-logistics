-- Motorcycle state-boundary guard.
-- Additive: this keeps the existing delivery, bicycle, rider and dispatch
-- behaviour, while making interstate motorcycle acceptance explicit.
begin;
...
declare
  ...
  interstate_dispatch boolean := false;
begin
  ...
  interstate_dispatch := coalesce(target_delivery.delivery_speed, '') = 'interstate'
    or lower(coalesce(target_delivery.metadata->>'interstate_dispatch', '')) = 'true';

  -- Interstate motorcycle jobs are deliberate long-haul jobs. They must start
  -- in the rider's registered pickup state; a nearby-border location cannot
  -- turn a local rider into an interstate rider.
  if interstate_dispatch and not pickup_matches_rider_state then
    raise exception 'Interstate motorcycle jobs must start in your registered rider state';
  end if;
  ...
  if bicycle_delivery then
    ...
    where assigned_rider_profile_id = target_rider.id and asset_type = 'bicycle' and status = 'available'
  ...
  update public.deliveries set
    status = 'accepted', rider_id = target_rider.id, accepted_at = coalesce(accepted_at, now()),
    fleet_asset_id = case when bicycle_delivery then target_bicycle.id else fleet_asset_id end,
    metadata = metadata || jsonb_build_object('offer_status', 'accepted', 'accepted_at', now(), 'accepted_rider_id', target_rider.id, 'fleet_asset_id', case when bicycle_delivery then target_bicycle.id else null end, 'fleet_asset_code', case when bicycle_delivery then target_bicycle.asset_code else null end),
    updated_at = now()
