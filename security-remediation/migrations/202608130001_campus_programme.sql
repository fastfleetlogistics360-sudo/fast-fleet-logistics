-- Additive KWASU campus-programme migration. Existing vendors, riders, and prices remain unchanged
-- until an administrator explicitly assigns a campus zone or lecturer benefit.

alter table if exists public.rider_profiles
  add column if not exists campus_zone_id text;

create index if not exists rider_profiles_online_campus_zone_idx
  on public.rider_profiles (online, campus_zone_id)
  where campus_zone_id is not null;
...
declare
  ...
  site_controls jsonb := '{}'::jsonb; rider_zone text; rider_state text; pickup_matches_rider_state boolean := false;
  cross_border_pickup_radius_km numeric := 10; location_freshness_minutes integer := 30; bicycle_max_route_km numeric := 10;
  rider_latitude numeric; rider_longitude numeric; rider_location_updated_at timestamptz; pickup_distance_km numeric; bicycle_delivery boolean := false;
  campus_priority_until timestamptz; campus_zone_id text;
begin
  ...
  if campus_zone_id is not null and campus_priority_until is not null and campus_priority_until > now() and coalesce(target_rider.campus_zone_id, '') <> campus_zone_id then
  ...
  if campus_zone_id is not null then bicycle_max_route_km := least(30, greatest(1, coalesce(nullif(target_delivery.metadata->>'campus_bicycle_cap_km', '')::numeric, bicycle_max_route_km))); end if;
  ...
  if bicycle_delivery then
    ...
    select * into target_bicycle from public.fleet_assets where assigned_rider_profile_id = target_rider.id and asset_type = 'bicycle' and status = 'available' order by updated_at asc limit 1 for update skip locked;
    if target_bicycle.id is null then raise exception 'This delivery requires an available assigned Fast Fleets bicycle'; end if;
  ...
  update public.deliveries set status = 'accepted', rider_id = target_rider.id, accepted_at = coalesce(accepted_at, now()), fleet_asset_id = case when bicycle_delivery then target_bicycle.id else fleet_asset_id end, metadata = metadata || jsonb_build_object('offer_status','accepted','accepted_at',now(),'accepted_rider_id',target_rider.id,'fleet_asset_id',case when bicycle_delivery then target_bicycle.id else null end,'fleet_asset_code',case when bicycle_delivery then target_bicycle.asset_code else null end), updated_at = now() where id = target_delivery.id;
  insert into public.delivery_locations (order_id, rider_id, latitude, longitude, heading, speed, status, updated_at) select target_delivery.id, target_rider.id, rl.latitude, rl.longitude, rl.heading, rl.speed, 'accepted', now() from public.rider_locations rl where rl.rider_profile_id = target_rider.id on conflict (order_id) do update set rider_id=excluded.rider_id, latitude=excluded.latitude, longitude=excluded.longitude, heading=excluded.heading, speed=excluded.speed, status=excluded.status, updated_at=excluded.updated_at;
  insert into public.delivery_events (delivery_id, actor_id, status, title, body) values (target_delivery.id, target_rider.user_id, 'accepted', 'Courier assigned', 'A verified courier accepted the order.');
